...
$query_to_get_details_of_members = "SELECT * FROM `olms_members` WHERE `mem_uname` = ?";
$stmt1 = $conn->prepare($query_to_get_details_of_members);
$stmt1->bind_param("s", $unme);
$stmt1->execute();
$member_result = $stmt1->get_result();
...
<head>
    ...
    <title><?php
            if (isset($mem_name)) {
                echo $mem_name;
            } else if (isset($lib_name)) {
                echo $lib_name;
            }
            ?></title>

    <script type="text/javascript">
        // Disable the back function
